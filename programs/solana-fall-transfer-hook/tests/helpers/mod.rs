...
pub fn setup() -> (LiteSVM, Keypair, Address) {
    ...
    let mover_program_id = token_mover::id();
    let mover_program_bytes = include_bytes!("../../../../target/deploy/token_mover.so");
    svm.add_program(mover_program_id, mover_program_bytes)
        .unwrap();
